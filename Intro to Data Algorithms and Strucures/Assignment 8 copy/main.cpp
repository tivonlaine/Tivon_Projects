/*
Name: Tivon Laine, 2002167255, Section 1002, Assignment 8
Description: Creating and fixing a timeline
Input: Timeline and Nexus Event
Output: Pruned timeline
*/
#include<iostream>
using namespace std;
class node{
...
class Timeline{
  public:
    ...
    int createNexusEvent(int amount = 1){
      ...
      node *temp=head;
      while(temp->year!=year&&temp!=nullptr&&temp->next!=nullptr){//making sure temp next is not empty
        temp=temp->next; }
    node *nexus=new node[2];
    if(temp!=nullptr&&temp->next!=nullptr){         //making sure temp is not empty
       ...
       int i=0;
          if(temp->branches>0){
            if(dir){
              ...
              arr[3][year+1]='/';  }     
              dir=!dir;
              while(temp2!=nullptr&&year+2+i<70){
                temp2=temp2->next;
                 arr[2][year+2+i]='~';
                i++;  }       }else{
            ...
            arr[6][year]='\\';
            arr[7][year+1]='\\';     }   
            while(temp3!=nullptr&&year+2+i<70){  
               temp3=temp3->next;
               arr[8][year+2+i]='~';
              i++;   }   dir=!dir;     }     }
          temp=temp->next;
    ...
    int prune(){ //prunes a branch and returns location it pruned. For all time. always.
      ...
      while(temp != nullptr){
        if(temp->branches>0){
          ...
          node* temp2 = temp->next[1].next;
          while(temp2 != nullptr){
          node* temp3 = temp2->next;
          delete temp2;
          temp2=temp3;
          branchSize++;    }
          node* current = new node(temp->next->year);
          current->branches = temp->next->branches;
          current->next= temp->next->next;
          delete [] temp->next;
          temp->next=current; break; }
        temp=temp->next;    }
      if(yearWithBranch != -1)
